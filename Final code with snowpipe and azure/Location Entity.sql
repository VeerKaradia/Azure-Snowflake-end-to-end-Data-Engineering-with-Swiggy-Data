SANDBOX.CONSUMPTION_SCH.CUSTOMER_DIMSANDBOX.CONSUMPTION_SCH.CUSTOMER_DIMSANDBOX.INFORMATION_SCHEMA.DATABASESSANDBOX.INFORMATION_SCHEMA.DATABASESSANDBOX.STAGE_SCH.LOCATIONuse role sysadmin;
...
-- creating a snowpipe so i can automaticall pull data froma azure blob in stage_Schma csv folder
...
-- WHERE PIPE_SCHEMA = 'STAGE_SCH' 
--   AND PIPE_NAME = 'LOCATION_PIPE';
-- -- when raw files were manually uploaded, so I need this 

-- select * from stage_sch.location

-- this code is used when working mannual from computer to uplaod file and storge in cvs_file folder
...
SHOW PIPES LIKE 'location_pipe' IN SCHEMA stage_sch;
...
SHOW TASKS LIKE 'location_merge_task2' IN SCHEMA clean_sch;
...
FROM TABLE(INFORMATION_SCHEMA.TASK_HISTORY(
    TASK_NAME => 'location_merge_task2',
    RESULT_LIMIT => 100
))
ORDER BY SCHEDULED_TIME DESC;
GRANT USAGE ON WAREHOUSE compute_wh TO ROLE <task_owner_role>;
SANDBOX.STAGE_SCH.LOCATION_PIPE
...
using(
        ...
        Case 
                ...
                case when City in ('Mumbai','Delhi', 'Bengaluru', 'Hyderabad', 'Chennai','Noida', 'Kolkata', 'Pune', 'Ahmedabad') then 'Tier-1'
                when City in ('Jaipur', 'Lucknow', 'Kanpur', 'Nagpur','Varanasi','Indore', 'Bhopal', 'Patna','Vadodara', 'Coimbatore',
                'Ludhiana', 'Agra', 'Nashik', 'Ranchi', 'Meerut','Allahganj','Moradabad','Bareilly','Aligarh', 'Raipur','Ghazipur', 'Guwahati', 'Chandigarh') then 'Tier-1'
                else 'Tier-3'
                end as city_tier,
                Cast(ZipCode as String) as Zip_Code,
                Cast(ActiveFlag as String) as Active_Flag,
                to_char(to_timestamp(CreatedDate, 'MM/DD/YYYY HH24:MI'), 'YYYY-MM-DD HH24:MI:SS') as created_ts,
                to_char(to_timestamp(ModifiedDate, 'MM/DD/YYYY HH24:MI'), 'YYYY-MM-DD HH24:MI:SS') as modified_ts,
                _stg_file_name,
                _stg_file_load_ts,
                _stg_file_md5,
                _copy_data_ts
...
CREATE OR REPLACE TASK consumption_sch.location_scd2_task2
WAREHOUSE = compute_wh
SCHEDULE = NULL
...
ALTER TASK consumption_sch.location_scd2_task2 RESUME;
...
select 
    ...
    t.$7::text as modifieddate,
    -- audit columns for tracking and debugging
    metadata$filename as _stg_file_name,SANDBOX.STAGE_SCH.LOCATION
    metadata$file_last_modified as _stg_file_load_ts,
    metadata$file_content_key as _stg_file_md5,
    current_timestamp as _copy_data_ts
...
--- making thing automatic so create a snowflake notification integration

SELECT 
 *
FROM SNOWFLAKE.ACCOUNT_USAGE.OBJECT_DEPENDENCIES
WHERE OBJECT_SCHEMA IN ('STAGE_SCH','CLEAN_SCH','CONSUMPTION_SCH')
   OR REFERENCED_OBJECT_SCHEMA IN ('STAGE_SCH','CLEAN_SCH','CONSUMPTION_SCH')
